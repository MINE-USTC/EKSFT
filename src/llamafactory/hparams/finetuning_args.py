...
@dataclass
class EKSFTArguments:
    r"""Arguments pertaining to the EKSFT (Entropy-KL Selective Fine-Tuning) algorithm."""

    use_eksft: bool = field(
        default=False,
        metadata={"help": "Whether or not to use the EKSFT algorithm for selective token fine-tuning."},
    )
    eksft_top_k_ratio: float = field(
        default=0.2,
        metadata={"help": "The ratio of tokens to select based on KL divergence and entropy."},
    )
    eksft_largest_kl: bool = field(
        default=True,
        metadata={"help": "Whether to select tokens with largest KL divergence (True) or smallest (False)."},
    )
    eksft_largest_entropy: bool = field(
        default=True,
        metadata={"help": "Whether to select tokens with largest entropy (True) or smallest (False)."},
    )
    eksft_lambda_entropy: float = field(
        default=1.0,
        metadata={"help": "The weight coefficient for entropy regularization loss."},
    )
    eksft_lambda_kl: float = field(
        default=0.5,
        metadata={"help": "The weight coefficient for KL divergence regularization loss."},
    )
    eksft_target_entropy: float = field(
        default=0.5,
        metadata={"help": "The target entropy threshold for dynamic lambda adjustment."},
    )
    eksft_target_kl: float = field(
        default=2.0,
        metadata={"help": "The target KL divergence threshold for dynamic lambda adjustment."},
    )
    eksft_is_union_mask: bool = field(
        default=False,
        metadata={"help": "Whether to use union (True) or intersection (False) of KL and entropy masks."},
    )
    eksft_output_dir: str | None = field(
        default=None,
        metadata={"help": "The output directory for saving KL and entropy logs during EKSFT training."},
    )
...
class FinetuningArguments(
    ...
    stage: Literal["pt", "sft", "rm", "ppo", "dpo", "kto", "eksft"] = field(
        default="sft",
